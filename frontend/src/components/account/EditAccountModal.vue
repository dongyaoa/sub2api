  ...
  >
    ...
    >
      ...
      <OpenAITLSFingerprintSettings
        v-if="account && account.platform === 'openai' && (account.type === 'oauth' || account.type === 'setup-token')"
      ...
      >
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          ...
          <div class="w-full flex-shrink-0 sm:w-56">
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    if (newAccount.type === 'oauth' || newAccount.type === 'setup-token') {
      ...
      codexFingerprintMode.value = (['off', 'single_device_multi_window', 'device', 'session', 'full'].includes(fpMode || '')
...
function loadQuotaControlSettings(account: Account) {
  ...
  if ((account.platform === 'openai' || account.platform === 'anthropic') &&
    ...
    const profileId = account.tls_fingerprint_profile_id ?? account.extra?.tls_fingerprint_profile_id
