<template>
  <div class="flex flex-col gap-0.5">
    <!-- 并发槽位 -->
    <CapacityBadge :color-class="concurrencyClass" :current="currentConcurrency" :max="account.concurrency" :progress="concurrencyProgress">
    ...
    <CapacityBadge
      ...
      :key="entry.proxy_id"
      :color-class="proxyPoolColorClass(entry)"
      :current="entry.current_concurrency ?? 0"
      :max="entry.concurrency"
      :progress="proxyPoolProgress(entry)"
...
const concurrencyProgress = computed(() => {
  const max = Number(props.account.concurrency) || 0
  if (max <= 0) return null
  return Math.min(100, (currentConcurrency.value / max) * 100)
})

const proxyPoolEntries = computed(() => props.account.proxy_pool ?? [])

const proxyPoolColorClass = (entry: NonNullable<Account['proxy_pool']>[number]) => {
  const current = entry.current_concurrency ?? 0
  if (current > 0) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
  return 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
}

const proxyPoolProgress = (entry: NonNullable<Account['proxy_pool']>[number]) => {
  const max = Number(entry.concurrency) || 0
  if (max <= 0) return null
  return Math.min(100, ((entry.current_concurrency ?? 0) / max) * 100)
}
