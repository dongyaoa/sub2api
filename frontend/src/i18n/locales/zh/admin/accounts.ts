export default {
    accounts: {
      stateTicket: {
  "title": "STATE 票据",
  "description": "仅为此账号开启。默认关闭，其他账号不受影响。",
  "enable": "启用此账号的 STATE 票据",
  "plan": "套餐",
  "planPro": "Pro（292）",
  "planTeam": "Team（332）",
  "planHint": "手动选择套餐。票据需符合所选长度，并通过实际返回模型和业务出口验证。更改套餐后需重新获取。",
  "globalOff": "全局总开关已关闭；可以保存账号设置，开启总开关后才会获取和使用 STATE。",
  "gatewaySettings": "前往网关服务设置",
  "globalPoolConfigured": "使用全局动态 IP 池：{address}",
  "globalPoolMissing": "尚未配置全局动态 IP 池，请先在网关服务设置中填写。",
  "globalPoolHint": "这里只控制本账号是否使用票据。采集统一使用全局 IP 池，复验和日常请求沿用本账号的固定代理或服务器直连。",
  "directRoute": "当前账号为无代理：票据复验和日常请求使用当前服务器直连，动态 IP 池仅用于采集。",
  "proxy": "采集代理",
  "proxyPlaceholder": "粘贴完整代理 URL（包含用户名和密码）",
  "proxyConfigured": "已配置：{address}。留空保存会保留现有连接。",
  "proxyHint": "仅用于获取 STATE。日常请求仍使用本账号上方已保存的固定代理；验证通过后才使用，约一小时有效并提前自动刷新。",
  "sidHint": "支持 HTTP / SOCKS5h 代理。动态代理用户名中的 {'{'}sid{'}'} 会在每次尝试时自动替换为新的会话标识。",
  "model": "目标模型：{model}",
  "ready": "可用，剩余 {time}",
  "savedTicket": "已保存票据",
  "capturedAt": "获取时间：{time}",
  "expiresAt": "到期时间：{time}",
  "savedTicketHint": "服务重启后票据仍会保留，无需复制原票据；这里只显示时间，不会泄露票据内容。",
  "refreshing": "正在续期，当前票据仍可用，剩余 {time}",
  "refreshFailedUsable": "本次续期失败，继续使用当前票据至到期。",
  "retryAfter": "将于 {time} 自动重试",
  "attempts": "已尝试 {count} 次",
  "watchdog": "动态守护",
  "watchdogEnabled": "已开启",
  "watchdogDisabled": "未开启",
  "watchdogHint": "业务响应的实际模型不符或返回长度为 312 的 STATE 时，会自动重新获取。312 仅作为实验异常信号。",
  "watchdogTriggerCount": "累计触发 {count} 次",
  "watchdogLastReason": "最近触发：{reason}",
  "watchdogModelMismatch": "实际返回模型不符",
  "watchdogState312": "312 长度异常信号",
  "fixedProxyUnsaved": "上方的固定代理尚未保存，请先保存账号并重新打开编辑窗口。",
  "unsaved": "票据设置尚未保存。先保存，再获取 STATE。",
  "save": "保存票据设置",
  "acquire": "立即获取",
  "reacquire": "重新获取",
  "failureHint": "启用后若没有有效 STATE，只暂停本账号目标模型的请求。关闭账号开关并保存即可恢复普通请求流程。",
  "saved": "票据设置已保存。",
  "loadFailed": "暂时无法读取票据状态，请重试。",
  "saveFailed": "保存失败，请检查动态 IP 池、账号状态与业务出口设置后重试。",
  "harvestFailed": "未能启动获取，请检查当前状态并重试。",
  "retry": "重试",
  "states": {
    "disabled": "未启用",
    "global_disabled": "全局总开关已关闭",
    "waiting": "等待获取",
    "harvesting": "正在获取并验证",
    "error": "获取失败"
  }
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '默认关闭，需手动开启。单机多窗口统一设备标识，各客户端会话和窗口保持稳定、独立的匿名标识；非 Codex 客户端仅匿名化缓存键，不生成 Codex 窗口标识。旧版设备+会话和完全收敛模式会合并会话，部分账号曾报告额度缩水。',
        codexFingerprintOff: '关闭（透传，默认）',
        codexFingerprintSingleDeviceMultiWindow: '单机多窗口',
        ...
        codexTurnTicket: 'STATE 票据',
        codexTurnTicketDesc: '仅显示手动启用账号的票据状态。',
        codexTurnTicketMissing: '等待 STATE',
        codexTurnTicketReady: '剩余 {time}',
        codexTurnTicketPaused: '等待 STATE，目标模型已暂停',
