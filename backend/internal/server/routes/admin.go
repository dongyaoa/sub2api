...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/recent-requests/batch", h.Admin.Account.GetBatchRecentRequests)
...
func registerChannelMonitorRoutes(admin *gin.RouterGroup, h *handler.Handlers, settingService *service.SettingService) {
	...
	monitors.Use(guard)
	{
		...
		monitors.GET("/display-order", h.Admin.ChannelMonitor.GetDisplayOrder)
		monitors.PUT("/display-order", h.Admin.ChannelMonitor.UpdateDisplayOrder)
