...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		// Durable custom tasks charge only after browser-playable content is stored;
		// use the official completion claim only for legacy/untracked tasks.
		if (endpoint == service.GrokMediaEndpointVideoStatus || endpoint == service.GrokMediaEndpointVideoContent) && videoRecord == nil {
