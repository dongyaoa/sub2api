...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		// Seedance and Grok use durable completion claims so repeated status polls
		// charge only once. Custom Grok tasks charge after playable content is stored;
		// the completion path below is only for legacy or untracked Grok tasks.
		...
		} else if (endpoint == service.GrokMediaEndpointVideoStatus || endpoint == service.GrokMediaEndpointVideoContent) && videoRecord == nil {
