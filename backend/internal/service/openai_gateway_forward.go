...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (recentResult *OpenAIForwardResult, recentErr error) {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if !isCompactRequest && !isCodexMultiWindowAccount(codexAccountIdentitySource(c, account)) && applyCodexClientMetadata(decoded, account) {
			markDecodedModified()
		}
		...
		if isCodexMultiWindowAccount(codexAccountIdentitySource(c, account)) {
			if applyCodexMultiWindowRequest(c, account, decoded) {
				markDecodedModified()
			}
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if err := s.applyOpenAICodexTicketToRequest(ctx, account, extractOpenAICodexTicketModel(body), req); err != nil {
		return nil, err
	}
	if account.UsesOpenAICodexProtocol() {
