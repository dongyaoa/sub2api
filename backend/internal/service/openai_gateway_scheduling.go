		...
		if stickyAccountID > 0 && stickyAccountID == account.ID && s.concurrencyService != nil {
			...
			if waitingCount < cfg.StickySessionMaxWaiting {
				return s.newSelectionResult(ctx, account, false, nil, &AccountWaitPlan{
					...
					MaxConcurrency: EffectiveAccountConcurrency(account),
...
func (s *OpenAIGatewayService) tryAcquireAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, account ...*Account) (*AcquireResult, error) {
	if s.concurrencyService == nil {
		return &AcquireResult{Acquired: true, ReleaseFunc: func() {}}, nil
	}
	if len(account) > 0 && account[0] != nil {
		return s.concurrencyService.AcquireAccountSlotForAccount(ctx, account[0])
	}
