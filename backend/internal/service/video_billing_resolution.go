...
// xAI 视频生成按秒计费，seconds 请求参数允许 1-15 秒；未指定时上游默认生成 4 秒。
// 计费时长必须与上游实际消耗对齐，否则用户可通过拉长 seconds 套利（提交时长由用户控制）。
const (
	VideoBillingMinDurationSeconds     = 1
	VideoBillingMaxDurationSeconds     = 15
	VideoBillingDefaultDurationSeconds = 4
)

// NormalizeVideoBillingDurationSecondsOrDefault 归一化计费用视频时长：
// 未指定（<=0）按上游默认 4 秒计，超出上游允许区间按边界收敛。
